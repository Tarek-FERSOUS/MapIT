...
Platforms such as ServiceNow, Jira Service Management, Confluence, GLPI, BookStack, Notion, SharePoint, and IT Glue show that the market already offers mature solutions for ticketing, documentation, and knowledge sharing. Their official product pages and documentation were used as the main reference points for this comparison \cite{servicenow_itsm,jira_service_management,confluence_docs,glpi,bookstack,notion,sharepoint,itglue}. The common pattern is that each tool is strong in one direction and less complete in the others: service desk control, collaborative writing, enterprise content management, or flexible note keeping. For MapIT, the relevant question is whether a tool keeps infrastructure documentation and incident knowledge close together without turning the workflow into an oversized enterprise suite.

\subsection{Visual positioning of the landscape}
The following schema summarizes the comparison logic used in this chapter. It keeps the review readable by showing how the market tools are filtered through the same evaluation criteria before the target solution is positioned.

\begin{figure}[H]
\centering
\fbox{\parbox{3.3cm}{\centering ServiceNow\\Jira Service Management\\Confluence}}
\hspace{0.4cm}$\rightarrow$\hspace{0.4cm}
\fbox{\parbox{3.8cm}{\centering Structured comparison\\traceability\\access control\\knowledge reuse}}
\hspace{0.4cm}$\rightarrow$\hspace{0.4cm}
\fbox{\parbox{3.0cm}{\centering MapIT\\focused academic scope}}
\caption{Visual positioning of the compared solutions}
\label{fig:solution-positioning}
\end{figure}
